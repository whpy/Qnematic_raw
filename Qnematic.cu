...
void field_visual(Qreal *f, string name, int Nx, int Ny){
    ofstream fval;
    string fname = name;
    fval.open(fname);
    for (int j=0; j<Ny; j++){
...
void print_spec(Qcomp* f, int Nxh, int Ny){
    for(int j = 0; j < Ny; j++){
        for (int i = 0; i < Nxh; i++){
            int index = i + j*Nxh;
...
void print_spec(Qreal* f, int Nxh, int Ny){
    for(int j = 0; j < Ny; j++){
        for (int i = 0; i < Nxh; i++){
            int index = i + j*Nxh;
            printf("%.2f  ", f[index]);
...
__global__
void vel_func(Qcomp *w_c, Qcomp *u_c, Qcomp *v_c, 
...
void coord(Qreal dx, Qreal dy, int Nx, int Ny){
...
// int main(){
//     Qcomp b = make_cuDoubleComplex(0,800);
//     Qcomp a = b/9*
// }
